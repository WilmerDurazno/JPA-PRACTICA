...
<c:set var="p" scope="request" value="${req}"/>
		...
		<div class="row">
			...
			<img style="height: 150px; width: 200px; padding-bottom: 20px;" alt="" src="http://localhost:8080/JPA-PRACTICA/diseno/imagenes/adm.png">
				<div class="dropdown">
				  ...
				  <div  class="dropdown-menu" aria-labelledby="dropdownMenuButton" >
				    <a  style = "font-size: 15px" class="dropdown-item" href="http://localhost:8080/JPA-PRACTICA/AnadirProductos?eid=<%=request.getParameter("eid") %>">Agregar Productos</a>
				    <a style = "font-size: 15px" class="dropdown-item" href="http://localhost:8080/JPA-PRACTICA/EliminarProducto?eid=<%=request.getParameter("eid") %>">Eliminar Producto</a>
				    <a style = "font-size: 15px" class="dropdown-item" href="http://localhost:8080/JPA-PRACTICA/EditarProducto?eid=<%=request.getParameter("eid") %>">Editar Producto</a>
				    <a style = "font-size: 15px" class="dropdown-item" href="http://localhost:8080/JPA-PRACTICA/BuscarProducto?eid=<%=request.getParameter("eid") %>">Buscar Productos</a>
				    <a style = "font-size: 15px" class="dropdown-item" href="http://localhost:8080/JPA-PRACTICA/ListarProductos?eid=<%=request.getParameter("eid") %>">Listar Producto</a>
				  </div>
				</div>
				<div class="dropdown">
				  <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
				   ...
				   <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
				    <a style = "font-size: 15px" class="dropdown-item" href="http://localhost:8080/JPA-PRACTICA/ListarReq?eid=<%=request.getParameter("eid") %>">Listar Ordenes</a>
			...
			<div style="width: 30px">
				...
				<tr>
					...
					<td style="color: white; font-size: 25px" scope="col"><strong>Aceptar</strong></td>
					<td style="color: white; font-size: 25px" scope="col"><strong>Rechazar</strong></td>
				</tr>
				<c:forEach var="p" items="${req}">
					<tr >
						<td style="color: white; font-size: 18px">${p.codigo}</td>
						<td style="color: white; font-size: 18px">${p.fecha}</td>
						<td style="color: white; font-size: 18px">${p.estado}</td>
						<td style="color: white; font-size: 18px">${p.pro_com_id.nombre}</td>
						<td style="font-size: 18px; color: blue;"><a onclick="location.reload()" href="http://localhost:8080/JPA-PRACTICA/Despachar?eid=<%=request.getParameter("eid")%>&cabecera=${p.codigo}&bandera=2">Aceptar</a></td>
						<td style="font-size: 18px; color: blue;"><a onclick="location.reload()" href="http://localhost:8080/JPA-PRACTICA/Despachar?eid=<%=request.getParameter("eid")%>&cabecera=${p.codigo}&bandera=1">Rechazar</a></td>
					</tr>
				</c:forEach>
			</table>
			</div>
